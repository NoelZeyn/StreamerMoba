      ...
      <div class="flex-1 overflow-y-auto p-6 custom-scrollbar">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
          <div v-for="(stat, index) in stats" :key="index" 
            ...
            <div class="flex items-center justify-between mb-4">
              <div :class="stat.iconBg" class="w-12 h-12 rounded-xl flex items-center justify-center group-hover:scale-110 transition-transform">
                <i :class="[stat.icon, stat.iconColor]" class="text-xl"></i>
              </div>
              <span :class="stat.trendColor" class="text-[10px] font-bold px-2 py-1 rounded-full bg-opacity-10">{{ stat.trend }}</span>
            </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div class="lg:col-span-2 space-y-6">
            <div class="bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
              <div class="flex items-center justify-between mb-6">
                <div>
                  <h2 class="text-lg font-bold text-gray-900 italic uppercase tracking-tighter">Recent Schedules</h2>
                  <p class="text-[10px] text-gray-400 font-bold uppercase tracking-widest">Page {{ currentPage }}</p>
                </div>
                
                <div class="flex items-center gap-2">
                  <button @click="fetchSchedules(pagination.prev_page_url)" :disabled="!pagination.prev_page_url || loading"
                  ...
                  <button @click="fetchSchedules(pagination.next_page_url)" :disabled="!pagination.next_page_url || loading"
                          class="p-2 bg-gray-50 rounded-lg hover:bg-blue-50 disabled:opacity-30 transition-colors border border-gray-100">
              ...
              <div class="space-y-4">
                <div v-if="loading" class="flex flex-col items-center py-12">
                  ...
                  <p class="text-[10px] font-bold text-gray-400 mt-4 uppercase tracking-widest">Synchronizing Data...</p>
                </div>

                <div v-else-if="schedules.length === 0" class="text-center py-10 bg-gray-50 rounded-xl border border-dashed border-gray-200">
                  <p class="text-sm text-gray-500 italic">No stream schedules found.</p>
                </div>
                ...
                <div v-else v-for="item in schedules" :key="item.id" 
                  ...
                  <div class="text-center min-w-[55px]">
                    <p class="text-[10px] font-bold text-blue-600 uppercase">{{ formatDay(item.start_time) }}</p>
                    <p class="text-xl font-black text-gray-900 leading-none">{{ formatDate(item.start_time) }}</p>
                  </div>
                  <div class="h-10 w-px bg-gray-200"></div>
                  <div class="flex-grow min-w-0">
                    <h4 class="text-sm font-bold text-gray-800 truncate uppercase tracking-tight">{{ item.title || 'Untitled Session' }}</h4>
                    <div class="flex items-center gap-2 mt-1">
                      <span class="text-[10px] text-gray-500 font-medium">{{ formatTime(item.start_time) }}</span>
          ...
          <div class="space-y-6">
            <div class="bg-gray-900 p-6 rounded-2xl shadow-xl text-white relative overflow-hidden">
              <div class="relative z-10">
                ...
                <div class="grid grid-cols-2 gap-3">
                  <router-link to="/schedules/create" class="bg-white/5 hover:bg-white/10 p-4 rounded-xl text-center border border-white/10 transition-all">
                    <i class="fas fa-plus-circle mb-2 block text-blue-400"></i>
                    <p class="text-[9px] font-bold uppercase tracking-widest">New Stream</p>
                  </router-link>
                  <router-link to="/players/create" class="bg-white/5 hover:bg-white/10 p-4 rounded-xl text-center border border-white/10 transition-all">
                    <i class="fas fa-user-plus mb-2 block text-purple-400"></i>
                    <p class="text-[9px] font-bold uppercase tracking-widest">Add Player</p>
                  </router-link>
                </div>
              </div>
            </div>
            ...
            <div class="bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
              <h2 class="text-sm font-bold text-gray-900 mb-6 uppercase tracking-widest italic border-l-4 border-green-500 pl-3">Active Players</h2>
              <div class="space-y-5">
                <div v-for="player in onlinePlayers" :key="player.id" class="flex items-center justify-between group">
                  <div class="flex items-center gap-3">
                    ...
                    <span class="text-xs font-bold text-gray-700 uppercase tracking-tight">{{ player.name }}</span>
                  </div>
                  <div class="flex flex-col items-end">
                    <span class="w-2 h-2 rounded-full bg-green-500 shadow-[0_0_8px_rgba(34,197,94,0.8)] animate-pulse"></span>
                    <span class="text-[8px] text-gray-300 font-black mt-1 uppercase">Live</span>
                  </div>
                </div>
              </div>
            </div>
  ...
  methods: {
    async fetchSchedules(url = '/api/v1/schedules') {
      ...
      try {
        ...
        // Laravel simplePaginate mengembalikan data di response.data.data
        const result = response.data.data;
        
        this.schedules = result.data; // Array data utama
