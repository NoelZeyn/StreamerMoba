        ...
        <section class="bg-gray-900 rounded-2xl p-6 text-white shadow-xl relative overflow-hidden">
          ...
          <div class="relative z-10 max-w-2xl">
            ...
            <div class="flex items-center gap-2 bg-white/10 p-1.5 rounded-xl border border-white/10 backdrop-blur-sm">
              ...
              <button @click="copyWebhook" class="bg-blue-600 hover:bg-blue-700 px-4 py-2 rounded-lg text-xs font-bold transition-all active:scale-95 flex items-center gap-2">
        ...
        <div v-else class="grid grid-cols-1 lg:grid-cols-3 gap-6">
          <div class="lg:col-span-2">
            <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden flex flex-col h-full">
              ...
              <div class="overflow-x-auto">
                <table class="w-full text-left border-collapse">
                  ...
                  <tbody class="divide-y divide-gray-100">
                    <tr v-for="donation in paginatedDonations" :key="donation.id" class="hover:bg-gray-50/50 transition-colors">
                      <td class="px-4 py-4">
                        <div class="flex flex-col">
                          <span class="text-xs font-bold text-gray-800">{{ donation.donator_name }}</span>
                          <span class="text-[9px] text-gray-400">ID: {{ donation.saweria_id || '-' }}</span>
                        </div>
                      </td>
                      <td class="px-4 py-4">
                      ...
                      <td class="px-4 py-4 text-center">
                        <span class="text-[10px] text-gray-400 font-medium uppercase">{{ formatTime(donation.created_at) }}</span>
                      </td>
                    </tr>
                  </tbody>
                </table>
              ...
              <div class="p-4 bg-gray-50 border-t border-gray-100 flex items-center justify-between mt-auto">
                <button @click="currentPage--" :disabled="currentPage === 1" class="p-2 disabled:opacity-30"><i class="fas fa-chevron-left text-xs"></i></button>
                <span class="text-[10px] font-bold text-gray-400 uppercase">Hal {{ currentPage }} / {{ totalPages }}</span>
                <button @click="currentPage++" :disabled="currentPage === totalPages" class="p-2 disabled:opacity-30"><i class="fas fa-chevron-right text-xs"></i></button>
              </div>
            </div>
          </div>
          ...
          <div class="space-y-6">
            <div class="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm">
              ...
              <ul class="space-y-4">
                <li class="flex gap-3">
                  <div class="w-5 h-5 bg-orange-100 text-orange-600 rounded-full flex items-center justify-center text-[10px] font-bold flex-shrink-0">1</div>
                  <p class="text-[11px] text-gray-600 leading-relaxed">Buka <a href="https://saweria.co/admin/integrations" target="_blank" class="text-blue-600 font-bold underline">Integration Saweria</a>.</p>
                </li>
                <li class="flex gap-3">
                  <div class="w-5 h-5 bg-orange-100 text-orange-600 rounded-full flex items-center justify-center text-[10px] font-bold flex-shrink-0">2</div>
                  <p class="text-[11px] text-gray-600 leading-relaxed">Klik tombol <b>Edit Webhook</b>.</p>
                </li>
                <li class="flex gap-3">
                  <div class="w-5 h-5 bg-orange-100 text-orange-600 rounded-full flex items-center justify-center text-[10px] font-bold flex-shrink-0">3</div>
                  <p class="text-[11px] text-gray-600 leading-relaxed">Paste URL Webhook Anda di kolom Endpoint URL dan Simpan.</p>
                </li>
              </ul>
            </div>

<div class="bg-blue-600 p-6 rounded-2xl text-white shadow-lg relative overflow-hidden group">
  <i class="fas fa-crown absolute -right-2 -bottom-2 text-6xl opacity-10 group-hover:scale-110 transition-transform"></i>
  ...
  <p class="text-[11px] leading-relaxed text-blue-50/80 mb-3">
    Sistem akan otomatis menambah <b>Play Balance</b> jika donatur menyertakan kata <span class="bg-white/20 px-1 rounded text-white font-mono">VIP</span> dan format ID di pesan Saweria.
  </p>
...
export default {
  ...
  data() {
    return {
      user: null, // Berisi data user termasuk webhook_token
      donations: [],
      loading: true,
      currentPage: 1,
      itemsPerPage: 8,
  ...
  methods: {
    async fetchData() {
      this.loading = true;
      try {
        // Ambil profil user untuk mendapatkan webhook_token
        const userRes = await axios.get('/api/v1/me'); 
        this.user = userRes.data;

        // Ambil riwayat donasi khusus user ini
