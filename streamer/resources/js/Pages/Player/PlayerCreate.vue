        ...
        <div class="max-w-4xl mx-auto">
          <form @submit.prevent="handleSubmit" class="grid grid-cols-1 lg:grid-cols-3 gap-6">
              ...
              <div class="bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
                ...
                <div class="space-y-5">
                  <div class="bg-blue-50/50 p-4 rounded-xl border border-blue-100 mb-2">
                    <label class="text-[10px] font-bold text-blue-400 uppercase mb-3 block tracking-wider">Cari via MLBB ID (Opsional)</label>
                  ...
                  <div>
                    <label class="text-[10px] font-bold text-gray-400 uppercase mb-2 block tracking-wider">Nama / Nickname di Sistem</label>
                    <input v-model="form.name" type="text" placeholder="Masukkan nama pemain..."
                      ...
                      required />
                    <p class="text-[9px] text-gray-400 mt-1 italic">*Jika menggunakan Saweria, pastikan nama ini sama dengan nama donatur.</p>
                  </div>

                  <div>
                    <label class="text-[10px] font-bold text-gray-400 uppercase mb-2 block tracking-wider">Tipe Keanggotaan</label>
                    <div class="grid grid-cols-2 gap-4">
                      <label :class="form.type === 'PUBLIC' ? 'border-purple-500 bg-purple-50 ring-1 ring-purple-500' : 'border-gray-100 hover:bg-gray-50 bg-white'"
                        ...
                        <i class="fas fa-globe text-lg" :class="form.type === 'PUBLIC' ? 'text-purple-600' : 'text-gray-300'"></i>
                        <span class="text-xs font-black uppercase tracking-tighter" :class="form.type === 'PUBLIC' ? 'text-purple-700' : 'text-gray-400'">Public</span>
                      </label>

                      <label :class="form.type === 'VIP' ? 'border-amber-500 bg-amber-50 ring-1 ring-amber-500' : 'border-gray-100 hover:bg-gray-50 bg-white'"
                        class="relative p-4 rounded-xl border-2 cursor-pointer transition-all flex flex-col items-center gap-2">
                        <input type="radio" v-model="form.type" value="VIP" class="hidden" />
                        <i class="fas fa-crown text-lg" :class="form.type === 'VIP' ? 'text-amber-500' : 'text-gray-300'"></i>
                        <span class="text-xs font-black uppercase tracking-tighter" :class="form.type === 'VIP' ? 'text-amber-700' : 'text-gray-400'">VIP Member</span>
                      </label>
                    </div>
                  </div>

                  <div v-if="form.type === 'VIP'" class="mt-5 animate-in slide-in-from-top-2 duration-300">
                    <label class="text-[10px] font-bold text-gray-400 uppercase mb-2 block tracking-wider">Credit Awal</label>
                    <div class="relative">
                      <span class="absolute left-4 top-1/2 -translate-y-1/2 text-gray-400 font-bold text-xs uppercase">Credit : </span>
            ...
            <div class="space-y-6">
              ...
              <div v-if="form.type === 'VIP'" class="bg-amber-900 text-white p-5 rounded-2xl shadow-lg shadow-amber-200/20">
                <div class="flex items-start gap-4">
                  ...
                  <div>
                    ...
                    <p class="text-[10px] text-amber-100/80 leading-relaxed mt-1">
                      Pemain ini sekarang bisa melakukan topup otomatis melalui Saweria dengan format nama atau ID MLBB yang sesuai.
                    </p>
                  </div>
                </div>
              </div>
...
export default {
  ...
  methods: {
    async fetchUserInfo() {
      try {
        // Ambil info user yang sedang login (untuk info 'Created By')
        const response = await axios.get('/api/v1/players'); // Atas asumsi endpoint ini me-return data user di relasinya
...
async fetchNickname() {
  ...
  try {
    ...
    // Menampilkan pesan error dari Laravel (Proxy)
    ...
    async handleSubmit() {
      ...
      try {
        const payload = {
          ...
          mlbb_id: this.form.mlbb_id,
          mlbb_server: this.form.mlbb_server
        };
